...
<template>
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="card overflow-hidden">
      <div v-if="loading" class="text-center py-12 text-[var(--color-text-muted)]">Cargando directorio...</div>
      
      <div v-else-if="students.length === 0" class="text-[var(--color-text-muted)] text-center py-16 flex flex-col items-center">
            ...
            <tr v-for="s in students" :key="s.id" class="hover:bg-[var(--color-bg-hover)] transition-colors group">
              <td class="px-6 py-4 font-medium">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-xs uppercase shrink-0">
                    {{ s.full_name ? s.full_name.substring(0, 2) : '?' }}
                  </div>
                  <div>
                    {{ s.full_name }}
